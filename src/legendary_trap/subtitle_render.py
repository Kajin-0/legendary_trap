...
def write_visual_ass(document: dict, path: Path, title: str = "FOCUS",
                     ...
                     lyric_bold: int = 1, artist_name: str | None = None,
                     artist_font: str = "Barlow Condensed", artist_title: str | None = None,
                     artist_offset_x: int = 72) -> None:
...
Style: Artist,{artist_font},27,&H00FFF9F0,&H00FFF9F0,&H00141A26,&H90070B12,1,0,1,2,1,7,72,72,62,1
Style: ArtistTitle,Barlow Condensed,18,&H00E9D8C8,&H00E9D8C8,&H00141A26,&H90070B12,1,0,1,1,1,7,72,72,62,1
...
"""
    ...
    if artist_name:
        rows.append(f"Dialogue: 0,0:00:00.00,0:00:18.00,Artist,,0,0,0,,{{\\an7\\pos({artist_offset_x},62)\\fad(180,220)}}{_ass_text(artist_name)}")
        if artist_title:
            rows.append(f"Dialogue: 0,0:00:00.00,0:00:18.00,ArtistTitle,,0,0,0,,{{\\an7\\pos({artist_offset_x},96)\\fad(180,220)}}{_ass_text(artist_title)}")
...
def write_subtitles(document: dict, output_dir: Path, title: str,
                    ...
                    lyric_bold: int = 1, artist_name: str | None = None,
                    artist_font: str = "Barlow Condensed", artist_title: str | None = None,
                    artist_offset_x: int = 72) -> dict[str, str]:
    ...
    write_visual_ass(document, ass, title, width, height, lyric_font, lyric_size, lyric_bold,
                     artist_name, artist_font, artist_title, artist_offset_x)
